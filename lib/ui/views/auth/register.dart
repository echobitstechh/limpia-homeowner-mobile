...
class _RegisterState extends State<Register> {
  ...
  Widget buildAutocompleteResults(bool isFullScreen,
      TextEditingController controller, AuthViewModel model) {
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return ListTile(
            ...
            onTap: () async {
              print('predition tap');
              print('predition text is: description ${prediction?.description}  lat ${prediction?.latitude}  long ${prediction?.longitude}'
                  'main ${prediction?.mainText}  sec ${prediction?.secondaryText} id ${prediction?.placeId}');
              final updatedPrediction =
              await MapsUtils().getPlaceDetails(prediction!);
              if (updatedPrediction != null) {
                print('place details is: description ${updatedPrediction?.description}  lat ${updatedPrediction?.latitude}  long ${updatedPrediction?.longitude}'
                    'main ${updatedPrediction?.mainText}  sec ${updatedPrediction?.secondaryText} id ${updatedPrediction?.placeId}');
                final secondaryTextParts = updatedPrediction.secondaryText
                    .split(',');
                ...
                if (secondaryTextParts.isNotEmpty) {
                  ...
                  if (secondaryTextParts.length > 1) {
                    model.stateValue = secondaryTextParts[secondaryTextParts.length - 2].trim();
                  }
                  model.countryValue = secondaryTextParts.last.trim();
                  model.streetValue = updatedPrediction.mainText;
                  model.lat = updatedPrediction.latitude;
                  model.long = updatedPrediction.longitude;

                }

                setState(() {
                  _selectedAddress = updatedPrediction;
                  print("selected address main is: ${_selectedAddress
                      ?.mainText}");
                  print("selected address secondary is: ${_selectedAddress
                      ?.secondaryText}");
                  print("selected address description is: ${_selectedAddress
                      ?.description}");
